...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void initState() {
    ...
    for (var element in bookings.bookings) {
      if (element.accel.name == 'VitalBeam1') {
        var calendarEvent = CalendarEvent(
          dateTimeRange:
              DateTimeRange(start: element.start, end: element.finish),
          eventData: Event(
              title:
                  "${element.patient.name}\t${formatter.format(element.start)}-${formatter.format(element.finish)}"),
        );
        calev.add(calendarEvent);
      }
    }
    eventController.addEvents(calev.cast());
  }
